    ...
    _configs()
{
  ...
  for ( int i = 0; i < 4; i++ ) {
    ...
    boost::random::uniform_real_distribution<> rtt( log( range.rtt_ms.first ), log( range.rtt_ms.second ) );
    boost::random::uniform_int_distribution<> num_senders( 1, range.max_senders );

    _configs.push_back( NetConfig().set_link_ppt( exp( link_speed( global_PRNG() ) ) ).set_delay( exp( rtt( global_PRNG() ) ) ).set_num_senders( num_senders( global_PRNG() ) ) );
  }
}
